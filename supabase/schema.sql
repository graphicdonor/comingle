...
-- Manual, viewer-initiated reports — separate from the automated pre-publish
-- AI pipeline. A report never hides the post itself (that's a human
-- decision, not an automatic takedown) — it queues the post into the same
-- admin moderation queue AI holds already use (see
-- enqueue_post_report_for_review below), so there's one review surface.
...
-- post more than one job, and it's a public directory, not a matching pool.
...
-- updated_at on job_listings is DB-owned so app code never sets it
...
-- useful aggregation is across *different* likers, so this batches by
-- (user_id, type, link) alone, updating actor_id to the most recent liker
-- each time so the UI can render "X and N others liked your post". No
-- decrement on unlike — retracting an already-sent notification isn't
...
-- Job listings: same shape as business_listings' moderation gating and
