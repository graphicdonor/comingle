...
export function JobListingOwnerActions({ listingId }: { listingId: string }) {
  ...
  const handleDelete = async () => {
    ...
    const { error: deleteError } = await supabase.from("job_listings").delete().eq("id", listingId);
    setLoading(false);
    ...
    return (
      <div className="flex flex-col items-end gap-1.5">
        <div className="flex items-center gap-2">
          <span className="text-xs text-gray-500">Delete this job posting?</span>
  ...
  return (
    <div className="flex items-center gap-3 flex-shrink-0">
      <Link
        href={`/services/jobs/${listingId}/edit`}
