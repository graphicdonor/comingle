...
import type { Post, Community, CommunityRole } from "@/lib/types";
...
export default async function CommunityPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [membershipResult, { data: posts }, likesResult] = await Promise.all([
    user
      ? supabase.from("community_members").select("role").eq("community_id", c.id).eq("user_id", user.id).maybeSingle()
  ...
  const viewerRole = (membershipResult.data?.role ?? null) as CommunityRole | null;
  const isMember = !!viewerRole;
  const canModerate = isCommunityStaff(viewerRole);
  ...
  return (
    <div className="max-w-2xl mx-auto">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-3">
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                {canModerate && (
                  <Link
                    href={`/communities/${c.slug}/manage`}
      ...
      <CommunityRules communityId={c.id} rules={c.rules} canEdit={viewerRole === "admin"} />
